...
Y = np.array([])
xList = []
for x in range(0, len(RainData)):  #xrange
...
Ymean = np.mean(Y)
Ynorm = np.array([])
for y in range(0, len(RainData)):  #xrange
...
exponentialconst, aprox =curve_fit(func, Xnorm, Ynorm)
a=exponentialconst[0];
b=exponentialconst[1];
c=exponentialconst[2];
 
def expfunc():
    return a, b, c

#Expplot
exponentialconstants, aprox =curve_fit(func, Xnorm, Ynorm);
plt.title('Recta resultante (Datos Normalizados)')
plt.plot(x, func(x, *exponentialconstants), 'g--')
plt.ion()
plt.grid(True)
plt.ylabel("Lluvia (mm)")
plt.xlabel("Atenuacion (dB)")
plt.savefig('Images/ExpAprox.png')
